...
export default function AdminDashboard() {
  ...
  const loadSavedData = async () => {
    ...
    } catch (err) { setMessage("❌ Error al cargar"); }
    setLoading(false);
  };

  const handleLogin = () => {
    if (password === "castle2024") { setLoggedIn(true); setMessage(""); }
    else { setMessage("❌ Contraseña incorrecta"); }
  };
  ...
  if (!loggedIn) {
    return (
      <div style={{ maxWidth: 400, margin: "100px auto", padding: 30, background: "#16213e", borderRadius: 10, textAlign: "center" }}>
        ...
        <button onClick={handleLogin} style={{ padding: "10px 30px", background: "#e94560", color: "#fff", border: "none", borderRadius: 5, cursor: "pointer" }}>Ingresar</button>
  ...
  const handleFileUpload = async (tourId: number, file: File) => {
    ...
    try {
      ...
      if (uploadError) { setMessage("❌ Error al subir: " + uploadError.message); setLoading(false); return; }
    ...
    } catch (err: any) { setMessage("❌ Error: " + (err.message || "Desconocido")); }
    setLoading(false);
    ...
    <div style={{ background: "#1a1a2e", minHeight: "100vh", color: "#eee", fontFamily: "Arial, sans-serif" }}>
      <div style={{ background: "#16213e", padding: "15px 30px", borderBottom: "2px solid #0f3460", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={loadSavedData} disabled={loading} style={{ background: "#0f3460", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 5, cursor: "pointer" }}>{loading ? "⏳" : "🔄"} Actualizar</button>
          <a href="/" style={{ color: "#eee", textDecoration: "none", background: "#0f3460", padding: "8px 16px", borderRadius: 5 }}>← Sitio web</a>
          <button onClick={() => setLoggedIn(false)} style={{ background: "#555", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 5, cursor: "pointer" }}>Salir</button>
        </div>
      </div>

      <div style={{ display: "flex", gap: 15, padding: "20px 30px", flexWrap: "wrap" }}>
        {[{ label: "Tours", value: stats.total }, { label: "Destacados", value: stats.featured }, { label: "Categorías", value: stats.categories }, { label: "Precio prom.", value: `$${stats.avgPrice.toLocaleString()}` }].map(s => (
      ...
      <div style={{ display: "flex", gap: 5, padding: "0 30px", marginBottom: 20, flexWrap: "wrap" }}>
        {[{ key: "tours", label: "🏔️ Tours" }, { key: "photos", label: "📸 Fotos" }, { key: "prices", label: "💰 Precios" }].map(tab => (
        ...
        {activeTab === "tours" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2 style={{ color: "#e94560", margin: 0 }}>Todos los tours ({tourList.length})</h2>
